...
* From our exploration, we found that the data were not stationary. Therefore, we need to correct for stationarity before modeling models that do not automatically difference the data or account for stationarity.
```{r}
...
check_accuracy <- function(modelname, quo_modelname, testdf){
    ...
    # bind wide forecasts for each month. 
...
```{r}
# holt winters forecast
hw_fc <- forecast(HoltWinters(train), h = length(test), damped = TRUE)

# plot, summarize, and store accuracy information from the forecast
accuracy_df  = plotsumm(hw_fc, "Holt Winters - Additive", test)
```
...
cbind('Residuals' = residuals(ses_fc_diff),
...
ses_fc_diff %>%autoplot(facets = TRUE) + ggtitle("South EU Passengers | 1 YR Forecast | SES alpha 0.78 - diff") + ylab("Passengers") + xlab("Time")
...
```{r}
# Construct Arima models using the results from eacf
Arima(train_diff,
      order = c(1,1,0))
...
# plot, summarize, and store accuracy information from the forecast
accuracy_df  = plotsumm(arima_model_fc, "ARIMA", test_diff)
```


#### auto.arima
* Notes:  
  + At lag 11 there is a spike in the ACF plot for the residuals. We expect this to be the multiple seasonality that we visualized in our EDA. 
...
mykable(accuracy_df[order(accuracy_df$fore_err),])
...
# limit data to prints we plan to present in class - all on same differenced scale for ease of interpretation
# if we want to compare the models that were also differenced for seasonality and trend, we can dedifference the data.
# Those models are not presented in our presentation for time and space reasons, and to focus on the models that we tuned most closely.
